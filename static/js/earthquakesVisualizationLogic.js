/****************************************************************************
 *
 *  File Name:  earthquakesVisualizationLogic.js
 *
 *  File Description:
 ...
 *      ChooseColorFromDepthFunction
 *      SetMarkerSizeFromMagitudeFunction
 ...
 *      DisplayTectonicPlatesSubroutine
 *      DisplayOrogensSubroutine
...
let tectonicPlatesUrlString = 'https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json';

let orogensUrlString = 'https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_orogens.json';
...
let tectonicPlatesOverlayLayerGroup
        = L.layerGroup();

let orogensOverlayLayerGroup
        = L.layerGroup();
...
const overlayLayerGroupDictionary 
        = {
              ...
              'Tectonic Plates': tectonicPlatesOverlayLayerGroup,
              'Orogens': orogensOverlayLayerGroup
          };
            ...
            ('mapid', 
                {
                    ...
                    layers: [outdoorsMapTileLayer,
                             ...
                             tectonicPlatesOverlayLayerGroup,
                             orogensOverlayLayerGroup]
                }
            );
...
/****************************************************************************
 *
 *  Function Name:  ChooseColorFromDepthFunction
 *
 *  Function Description:  
...
function ChooseColorFromDepthFunction
            (depthFloat) 
{
    if (depthFloat >= depthRangeFloatArray[5])
    {
        return 'red';
    }
    else if (depthFloat >= depthRangeFloatArray[4])
    {
        return 'orangered';
    }
    else if (depthFloat >= depthRangeFloatArray[3])
    {
        return 'orange';
    }
    else if (depthFloat >= depthRangeFloatArray[2])
    {
        return 'gold';
    }
    else if (depthFloat >= depthRangeFloatArray[1])
    {
        return 'yellow';
    }
    else
...
/****************************************************************************
 *
 *  Function Name:  SetMarkerSizeFromMagitudeFunction
 *
 *  Function Description:  
...
function SetMarkerSizeFromMagitudeFunction
            (magnitudeFloat) 
{
    return Math.sqrt(Math.abs(magnitudeFloat))*70000.0;
} // This right brace ends the block for the function, 
// SetMarkerSizeFromMagitudeFunction.
...
/****************************************************************************
 *
 *  Subroutine Name:  DisplayTectonicPlatesSubroutine
 *
 *  Subroutine Description:  
...
function DisplayTectonicPlatesSubroutine()
{
    var layerExistsBoolean
            = true;

    if (currentMapObject.hasLayer(tectonicPlatesOverlayLayerGroup) == false)
    {
        layerExistsBoolean = false;
    }

    tectonicPlatesOverlayLayerGroup.clearLayers();

    tectonicPlatesOverlayLayerGroup.addTo(currentMapObject);

    FetchJsonDataFromURLFunction 
        (tectonicPlatesUrlString)
            .then
            (
                (tectonicGeoJsonDictionary => 
                    {
                        L.geoJSON
                        (
                            tectonicGeoJsonDictionary.features, 
                            {style: 
                                {color: 'firebrick',
                                 weight: 4.0}
                            }
                        )
                        .addTo(tectonicPlatesOverlayLayerGroup);
...
/****************************************************************************
 *
 *  Subroutine Name:  DisplayOrogensSubroutine
 *  Subroutine Description:  
...
function DisplayOrogensSubroutine()
{
    var layerExistsBoolean
            = true;

    if (currentMapObject.hasLayer(orogensOverlayLayerGroup) == false)
    {
        layerExistsBoolean = false;
    }

    orogensOverlayLayerGroup.clearLayers();

    orogensOverlayLayerGroup.addTo(currentMapObject);

    FetchJsonDataFromURLFunction 
        (orogensUrlString)
            .then
            (
                (orogensGeoJsonDictionary => 
                    {
                        L.geoJSON
                        (
                            orogensGeoJsonDictionary.features, 
                            {style: 
                                {color: 'steelblue',
                                 weight: 2.0}
                            }
                        )
                        .addTo(orogensOverlayLayerGroup);
                                    ...
                                    .filter
                                        (featuresKey =>
                                            (Number(featuresKey.properties.mag).toFixed(1) 
                                                >= magnitudeDropDownMenuOptionsDictionary[magnitudeDictionaryKeyString][0]
                                             && Number(featuresKey.properties.mag).toFixed(1) 
                                                <= magnitudeDropDownMenuOptionsDictionary[magnitudeDictionaryKeyString][1]))
                                    .filter
                                        (featuresKey =>
                                            (Number(featuresKey.geometry.coordinates[2]).toFixed(1) 
                                                >= depthDropDownMenuOptionsDictionary[depthDictionaryKeyString][0]
                                             && Number(featuresKey.geometry.coordinates[2]).toFixed(1) 
                                                <= depthDropDownMenuOptionsDictionary[depthDictionaryKeyString][1]));
                        ...
                        {   
                            ...
                            (
                                ([currentEarthquakeFeaturesDictionaryArray[i].geometry.coordinates[1],
                                 ...
                                 {
                                    radius: SetMarkerSizeFromMagitudeFunction
                                                (currentEarthquakeFeaturesDictionaryArray[i].properties.mag),
                                    fillColor: ChooseColorFromDepthFunction
                                                (currentEarthquakeFeaturesDictionaryArray[i].geometry.coordinates[2]),
                                    fillOpacity: 0.7,
                                    color: 'black',
                                    stroke: true,
                                    weight: 0.5
            ...
            = function() 
              {
                    ...
                    divDOMUtilityObject.innerHTML 
                        = '<p>Depth</p>' 
                          + depthRangeFloatArray.map
                                (function(elementFloat, indexInteger) 
                                 {
                                    return '<span style = "background:' 
                                           + ChooseColorFromDepthFunction(elementFloat + 1.0) 
                                           + '">&nbsp &nbsp &nbsp &nbsp</span> ' 
                                           + elementFloat 
                                           + (depthRangeFloatArray[indexInteger + 1] 
                                              ? ' &ndash; ' + depthRangeFloatArray[indexInteger + 1] + '<br>' 
                                              : '+');
                                 }
                                ).join('');
